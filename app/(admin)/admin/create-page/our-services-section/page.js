...
import { Alert, Button, Input, List, message, Select, Switch, Upload } from "antd";
...
export default function OurServicesSection() {
	...
	return (
		<>
			<div className="w-full flex flex-col gap-4 justify-center items-center p-2">
				<Input className="hover:cursor-pointer " placeholder="The Url of the current Page" readOnly variant="filled" value={url} onClick={(e) => setUrl(e.target.value)} />
				...
				<h2 className="text-3xl font-poppins text-center font-semibold my-4 py-2 bg-gray-100/60 shadow-sm shadow-black/10 text-orange-400 rounded-lg  w-full "> Services </h2>
      ...
      className="mt-4 flex items-center justify-center w-12 h-12 bg-blue-500 text-white rounded-full shadow-lg hover:bg-blue-600 transition-transform transform hover:scale-110"
      aria-label="Add"
    >
    ...
    renderItem={(item, index) => (
      <List.Item className=" ">
        <List.Item.Meta
          avatar={<p className="w-6 h-6 rounded-full bg-orange-400 text-white font-semibold ">{index+1}</p>}
          title={item.title}
