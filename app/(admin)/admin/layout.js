...
export default function RootLayout({ children }) {
	...
	const [defaultSelectedKeys, setDefaultSelectedKeys] = useState(["dashboard-view"]);
	const [defaultOpenKeys, setDefaultOpenKeys] = useState(["dashboard-view"]);
    ...
    <GlobalLoader loading={loading || isPending} />
		<Layout className="w-[100vw]   ">
			...
			>
				...
				<h2 className="text-gray-200 font-poppins text-lg tracking-wide">
					Welcome to Admin Panel | Affotax
				</h2>
				<div>
				...
				>
					...
					>
						<OrdersContext.Provider
							value={{ unread_count, set_unread_count }}
						>
							{children}
						</OrdersContext.Provider>
					</Content>
			...
			>
				<section className="flex justify-between items-center px-2 font-poppins ">
					<div>
						{!isSider && (
							<RiMenuUnfoldLine
								className="scale-125 text-black/75  hover:text-orange-400 hover:scale-150 transition-all cursor-pointer border  rounded-sm shadow-sm shadow-black/20"
					...
					<Link
						href={"https://affotax.com"}
						target="_blank"
						className="text-black hover:text-orange-400 ml-56"
					>
						<span className="font-semibold ">
							Affotax ©{new Date().getFullYear()}{" "}
						</span>
					</Link>
