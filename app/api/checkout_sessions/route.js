...
export async function POST(req) {
    const { cartItemsArr, totalPrice, selectedPriceIds } = await req.json();  // Assuming you send items in the request body as array
 ...
 const db = await connectDB();
       ...
       let newOrderData = {
        items: cartItemsArr.map((el) => ({priceId: el._id, pageTitle: el.pageTitle, priceTitle: el.priceTitle, price: el.price, addOns: el.addOns?.filter((addOn) => addOn.isChecked === true)})),
...
// export async function POST(req) {
//   const { cartItemsArr } = await req.json();  // Assuming you send items in the request body as array
...
//         unit_amount: ((+(item.price)).toFixed(2)) * 100, // Amount in cents
//       },
//       quantity: item.quantity || 1,
//     })),
//     mode: 'payment',
