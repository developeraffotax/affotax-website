						...
						{blogs.map((el) => {
							return (
								<article className="max-w-xs" key={el._id}>
									...
									<Link href={`/blog/${el.slug}`} className="inline-flex items-center font-medium underline underline-offset-4 text-primary-600 dark:text-primary-500 hover:no-underline" > Read in 5 minutes </Link>
								</article>
							);
						})}
					</div>
