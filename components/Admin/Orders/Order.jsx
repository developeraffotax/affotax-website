...
import { Badge, ConfigProvider, Input, List, Popconfirm, Space, Spin, Table, Tag, Typography } from 'antd';
...
export default function Order({orderData}) {
    ...
    // const [orderedItems, setOrderedItems] = useState(JSON.parse(orderData));
    ...
    const columns = [
        ...
        //   dataIndex: 'orderNumber',
          key: 'item_name',
          render: (text, record) => <p className='font-semibold '>{`${record.priceTitle} | ${record.pageTitle}`}</p>,
      ...
      const expandedRowRender = (record, index, indent, expanded) => {
        ...
        header={<h3 className='font-semibold text-base w-full text-start bg-orange-100 text-orange-800  p-1 px-4   '>AddOns for {record.pageTitle}</h3>}
          ...
          <List.Item className='p-0 '>
                ...
                <table className=''  >
                    {/* <thead>
                        <tr >
                            <td colSpan={2}>
                                    <h3 className='font-poppins font-semibold text-base w-full text-center bg-orange-100 text-orange-800  p-2   '>AddOns for {record.pageTitle}</h3>
                                </td> </tr>
                    </thead> */}
                        ...
                        <tr className='w-full grid grid-cols-2 justify-start gap-4 items-start  text-xs font-poppins    '>
                            <td className=' font-[500]   text-start lg:px-4   '>{index + 1}. {item.priceTitle}</td>
                            <td className=' font-[500]   text-end lg:px-4 '>£{item.price}</td>
                        </tr>
...
<table className='border-collapse border w-full max-w-lg mb-4 '   >
            <thead>
                <tr>
                    <td colSpan={2}>
                        <h3 className='font-semibold text-lg w-full text-start bg-orange-100 text-orange-800  p-1 px-4 '>Order Details</h3>
                    </td>
                </tr>
            </thead>
            ...
            <tbody >
                ...
                <tr className=' font-poppins border-b  font-[500] '>
                    ...
                    <td className='bg-gray-100 px-4 py-2 w-[50%]  '>{order.orderNumber}</td>
                </tr>

                 <tr className=' font-poppins border-b  font-[500]  '>
                    <td className='px-2'>Payment Status</td>
                    <td className='bg-gray-100 px-4 py-2 w-[50%]  '>{order.payment_status}</td>
                </tr>

                 <tr className=' font-poppins border-b  font-[500]  '>
                    <td className='px-2'>Total Price without VAT</td>
                    <td className='bg-gray-100 px-4 py-2 w-[50%]  '>£{order.totalPriceWithoutVat}</td>
                </tr>

                 <tr className=' font-poppins border-b  font-[500]  '>
                    <td className='px-2'>VAT | 20%</td>
                    <td className='bg-gray-100 px-4 py-2 w-[50%]  '>£{order.vat}</td>
                </tr>

                 <tr className=' font-poppins border-b  font-[500]  '>
                    <td className='px-2'>Total Price</td>
                    <td className='bg-gray-100 px-4 py-2 w-[50%]  '>£{order.totalPrice}</td>
                </tr>
...
<h2 className='font-poppins font-semibold text-base w-full text-center bg-orange-100 text-black  p-2   '>Ordered Items</h2>
<Table  className=''  columns={columns} expandable={{
        // expandedRowKeys: ['0'],
        expandedRowRender: expandedRowRender,
        //   defaultExpandedRowKeys: ['0'],
      }}
      
       dataSource={orderItems} size='large' showHeader bordered  />
