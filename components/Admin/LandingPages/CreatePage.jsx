...
import { createNewPage, deleteNewPage } from "@/actions/landingpage";
...
export default function CreatePage({pageSlug}) {
	...
	const [url, setUrl] = useState(() => {
		...
		return window?.location?.origin + "/services"  + "/" + pageSlug;
	});
	...
	useEffect(() => {
		if (pageSlug) {
			...
			getPageData();
			localStorage?.setItem('landing-page-url', url)
			console.log(url)

		} else {
			setUrl(window?.location?.origin + "/services" + "/");
		}
	}, []);
	...
	const submitHandler = async () => {
		...
		if (res.success) {
			...
			message.success(`${isEditMode ? 'Landing Page Updated' : 'New Landing Page Added Successfully'}`);
	...
	const getPageData = async () => {
        ...
        try {
			...
			let res = await axios.get(`/api/landing-pages/${url.split('/')[4]}`);
			...
			<Input className="" placeholder="The Url of the current Page" variant="filled" value={url}  onChange={(e) => setUrl(e.target.value)} />
				...
				<div className="w-full flex gap-8 justify-center items-start ">
					<div className="w-full flex flex-col gap-2 ">
						...
						<Input className=" " placeholder="Title of the Page" value={title} onChange={(e) => { setTitle(e.target.value); const slug = e.target.value .toLocaleLowerCase() .trim() .replaceAll(" ", "-") .replace(/[^\w\-]/g, ""); setSlug(slug); const url = window.location.origin  + "/services" + "/" + slug; setUrl(url); }} />
					</div>
				...
				<div className="w-full flex flex-row items-center justify-between  ">
					<Button className="w-[10%] min-w-44 mt-4" onClick={submitHandler} type="primary" >{isEditMode ? 'Update Landing Page' : 'Add New Landing Page'} </Button>
					{isEditMode && <Button className="w-[10%] min-w-44 mt-4" onClick={dltPageBtnHandler} type="primary" danger> {" "} Delete this Landing Page{" "} </Button>}
				</div>
