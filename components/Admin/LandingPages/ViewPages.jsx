...
import { deleteNewPage } from "@/actions/landingpage";
...
export default function ViewPages() {
    ...
    const getAllPages = async () => {
        ...
        setIsLoading(true);
        try {
            const res = await axios.get('/api/landing-pages/get-all-pages');
    ...
    const columns = [
        ...
        
        {
          ...
          render: (_, record) => {
                console.log(_)
            return <Space size="middle"> <Link href={`/services/${record.slug}`} target='_blank' ><AiOutlineEye className='text-sky-500 scale-150 active:scale-125 hover:scale-[1.7] transition-all' /></Link> <Link href={`/admin/landing-page/create?page=${record.slug}`}><BiEdit className='text-green-500 scale-150 active:scale-125 hover:scale-[1.7] transition-all'/></Link> <button onClick={() => showModal(record.slug)}><RiDeleteBin6Line className='text-red-500 scale-150 active:scale-125 hover:scale-[1.7] transition-all' /></button> <Modal
            open={open}
            title="Are you sure?"
            onOk={() => deletePage()}
            okType='danger'
