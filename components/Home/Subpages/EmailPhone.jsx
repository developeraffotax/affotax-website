        ...
        <section className=' w-full bg-gray-50 px-60 max-2xl:px-40 max-xl:px-10'>
            ...
            <div className=' container mx-auto  flex justify-around items-center py-10 w-full border-b-2     max-lg:flex-col max-lg:gap-2  bg '>
                <div className='flex items-center justify-center  '>
                    <h3 className='text-3xl text-tertiary font-semibold z-20 max-lg:text-2xl relative text-center'>
                ...
                <div>
                <button className='drop-shadow-md transition-all inline-flex text-secondary bg-primary border-0 py-2 px-6 hover:outline-primary outline-1 outline outline-transparent hover:bg-transparent hover:text-primary rounded-md text-lg font-semibold hover:scale-105 active:scale-95  '> Live Chat </button>
                <button className='ml-3 drop-shadow-md transition-all inline-flex text-secondary bg-primary border-0 py-2 px-6 hover:outline-primary outline-1 outline outline-transparent hover:bg-transparent hover:text-primary rounded-md text-lg font-semibold hover:scale-105 active:scale-95  '> Get a Quote </button>
                </div>
            </div>
                        ...
                        contactArr.map((el) => {
                            return (
                                <li key={el.name} className='flex flex-col justify-center gap-2 items-center p-4 border-t-0 border-b-0 border-r-primary/60 border-l-primary/60  border-2 rounded-xl  '>
                                    ...
                                    <Link href={el.link}>
                                    <div className='flex justify-center font-semibold  items-center gap-1 text-black text-xl hover:scale-105 transition-transform'>
                                        {el.icon}
                                        <h3>{el.value}</h3>
                                    </div>
                                    </Link>
                                    <p className='text-gray-500'>{el.timings}</p>
                                </li>
                            )
                        })
                    }
